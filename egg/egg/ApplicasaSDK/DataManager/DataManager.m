...
- (void) respondToGetArray_ResponseType:(NSInteger)responseType ResponseMessage:(NSString *)responseMessage Array:(NSArray *)array Block:(void *)block{
    ...
    GetDataManagerArrayFinished _block = (__bridge GetDataManagerArrayFinished)block;
...
- (id) init {
	if (self = [super init]) {
		...
		dataManagerLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		dataManagerLastUpdate               = [item objectForKey:KeyWithHeader(KEY_dataManagerLastUpdate, header)];
...
- (id) initWithObject:(DataManager *)object {
	if (self = [super init]) {
		...
		dataManagerLastUpdate               = object.dataManagerLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			dataManagerLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][DataManagerLastUpdateIndex])]];
