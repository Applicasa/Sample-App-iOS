...
- (void) respondToGetArray_ResponseType:(NSInteger)responseType ResponseMessage:(NSString *)responseMessage Array:(NSArray *)array Block:(void *)block{
    ...
    GetGameVArrayFinished _block = (__bridge GetGameVArrayFinished)block;
    _block(error,array);
}
...
- (id) init {
	if (self = [super init]) {
		...
		self.gameVID				= @"0";
		gameVLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
		self.gameVValue				= 0;
		self.gameVFunction				= @"";
	}
	return self;
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		gameVLastUpdate               = [item objectForKey:KeyWithHeader(KEY_gameVLastUpdate, header)];
...
- (id) initWithObject:(GameV *)object {
	if (self = [super init]) {
		...
		gameVLastUpdate               = object.gameVLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			gameVLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][GameVLastUpdateIndex])]];
