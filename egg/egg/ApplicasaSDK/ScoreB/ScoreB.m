...
- (id) init {
	if (self = [super init]) {
		...
		self.scoreBID				= @"0";
		scoreBLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
		self.scoreBScore				= 0;
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		scoreBLastUpdate               = [item objectForKey:KeyWithHeader(KEY_scoreBLastUpdate, header)];
...
- (id) initWithObject:(ScoreB *)object {
	if (self = [super init]) {
		...
		scoreBLastUpdate               = object.scoreBLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			scoreBLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][ScoreBLastUpdateIndex])]];
