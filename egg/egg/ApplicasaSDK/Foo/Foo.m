...
- (void) respondToGetArray_ResponseType:(NSInteger)responseType ResponseMessage:(NSString *)responseMessage Array:(NSArray *)array Block:(void *)block{
    ...
    GetFooArrayFinished _block = (__bridge GetFooArrayFinished)block;
...
- (id) init {
	if (self = [super init]) {
		...
		self.fooID				= @"0";
		fooLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
		self.fooName				= @"";
		self.fooDescription				= @"";
		self.fooBoolean				= YES;
		self.fooDate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
		self.fooImage				= [NSURL URLWithString:@""];
		self.fooFile				= [NSURL URLWithString:@""];
		self.fooLocation				=  [[CLLocation alloc] initWithLatitude:0 longitude:0];
		self.fooNumber				= 0;
		self.fooAge				= 0;
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		fooLastUpdate               = [item objectForKey:KeyWithHeader(KEY_fooLastUpdate, header)];
		...
		self.fooLocation               = [[CLLocation alloc] initWithLatitude:[[item objectForKey:KeyWithHeader(KEY_fooLocationLat, header)] floatValue] longitude:[[item objectForKey:KeyWithHeader(KEY_fooLocationLong, header)] floatValue]];
...
- (id) initWithObject:(Foo *)object {
	if (self = [super init]) {
		...
		fooLastUpdate               = object.fooLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			fooLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][FooLastUpdateIndex])]];
...
;
			self.fooLocation =  [[CLLocation alloc] initWithLatitude:sqlite3_column_double(stmt, array[0][FooLocationLatIndex]) longitude:sqlite3_column_double(stmt, array[0][FooLocationLongIndex])];
