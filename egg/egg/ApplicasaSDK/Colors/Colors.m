...
- (void) respondToGetArray_ResponseType:(NSInteger)responseType ResponseMessage:(NSString *)responseMessage Array:(NSArray *)array Block:(void *)block{
    ...
    GetColorsArrayFinished _block = (__bridge GetColorsArrayFinished)block;
    _block(error,array);
}
...
- (id) init {
	if (self = [super init]) {
		...
		self.colorsID				= @"0";
		colorsLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
		self.colorsNumber				= 0;
		self.colorsColor				= @"";
	}
	return self;
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		colorsLastUpdate               = [item objectForKey:KeyWithHeader(KEY_colorsLastUpdate, header)];
...
- (id) initWithObject:(Colors *)object {
	if (self = [super init]) {
		...
		colorsLastUpdate               = object.colorsLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			colorsLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][ColorsLastUpdateIndex])]];
