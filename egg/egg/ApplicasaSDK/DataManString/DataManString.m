...
- (void) respondToGetArray_ResponseType:(NSInteger)responseType ResponseMessage:(NSString *)responseMessage Array:(NSArray *)array Block:(void *)block{
    ...
    GetDataManStringArrayFinished _block = (__bridge GetDataManStringArrayFinished)block;
    _block(error,array);
}
...
- (id) init {
	if (self = [super init]) {
		...
		dataManStringLastUpdate				= [[NSDate alloc] initWithTimeIntervalSince1970:0];
...
- (id) initWithDictionary:(NSDictionary *)item Header:(NSString *)header{
	if (self = [self init]) {
		...
		dataManStringLastUpdate               = [item objectForKey:KeyWithHeader(KEY_dataManStringLastUpdate, header)];
...
- (id) initWithObject:(DataManString *)object {
	if (self = [super init]) {
		...
		dataManStringLastUpdate               = object.dataManStringLastUpdate;
...
- (id) initWithStatement:(sqlite3_stmt *)stmt Array:(int **)array IsFK:(BOOL)isFK{
	if (self = [super init]){
			...
			dataManStringLastUpdate = [[LiCore liSqliteDateFormatter] dateFromString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, array[0][DataManStringLastUpdateIndex])]];
